...
def run(D, t, L, tilt, F_hub, M_hub):
    ...
    imb1  = inode[n-2] # Adjust index of where first main bearing is located
    imb2  = inode[n-4] # Adjust index of where second main bearing is located
    ...
    Ryy = np.array([RIGID,  FREE, FREE]) # downwind bearing carry moments
    Rzz = np.array([RIGID,  FREE, FREE]) # downwind bearing carry moments
    ...
    for iframe, myframe in enumerate([myframe_1mb, myframe_2mb]):
        print('\n\n'+'-'*50)
        print('Running analysis with',iframe+1,'main bearings:')
        print('-'*50)
...
if __name__ == '__main__':
    ...
    run(diameter, thickness, length, tilt, F_hub, M_hub)
